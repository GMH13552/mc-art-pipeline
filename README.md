...
> **已归档 —— 被 [mc-art](https://github.com/GMH13552/mc-art) 取代。**
>
> 这里的架构让脚本自己调用大模型:路由、规划、绘制、盲审、修复全在进程内完成。
> 实测下来这条路代价高、判得也不准:
>
> - 每个成员约 10 次调用、每次 45–90 秒,时间几乎全在 thinking token 上
>   (单次 11k–33k,同样的输入耗时差 2.7 倍);
> - 用来验收的视觉模型把 16×16 的弓认成 *trident*、把时钟认成 *music disc*,
>   修复循环随后一直追这个噪声;
> - 有整整两个家族成员死在"推理把输出预算吃光",一个死在截断的 JSON。
>
> `mc-art` 保留了这里的确定性部分(扫描、光栅化、度量、打包),
> 把每一个需要判断的决策交还给调用它的模型。
>
> 本仓库保留作为基线,以及 plan 格式的样例来源。
